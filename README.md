...
2. **각국 공식 소스** — 한국은행 ECOS(국고채 전 만기·회사채), 일본 재무성(JGB 전 만기).
3. **FRED** — 미국 국채 커브, ICE BofA 등급별 회사채, 환율. API 키 없이 전체 히스토리.
4. **CNBC / 네이버** — 오늘자 시세만. 위 소스가 아직 못 따라온 최근 하루를 덮어쓴다.
5. **SEC EDGAR** — 회사채 발행 조건(FWP pricing term sheet).
